...
function handleFormSubmit(e) {
  try {
    ...
    if (studentEmail) {
      ...
      try {
        const adminEmail = "ykebed40@gmail.com";
        const adminHtml = buildAdminSummaryHtml(cfg.examName, studentName, studentEmail, resultsForEmail);
        MailApp.sendEmail({
          to: adminEmail,
          subject: `Admin Report: ${studentName} - ${cfg.examName}`,
          htmlBody: adminHtml
        });
        Logger.log("Admin summary email sent to: " + adminEmail);
      } catch (adminErr) {
        Logger.log("Failed to send admin summary: " + adminErr);
      }
    }
...
/** Build an admin-facing summary email. */
function buildAdminSummaryHtml(examName, studentName, studentEmail, results) {
  const totalScore = results.reduce((sum, r) => sum + (r.score || 0), 0);
  
  // Identify Strengths and Weaknesses
  const sorted = [...results].sort((a, b) => b.score - a.score);
  const strengths = sorted.filter(r => r.score >= 8); // Assume 8+ is strong
  const weaknesses = sorted.filter(r => r.score < 5);  // Assume <5 is weak

  let strengthsHtml = strengths.length > 0 
    ? strengths.map(s => `<li><strong>${s.qHeader}</strong> (Score: ${s.score})</li>`).join("")
    : "<li>No specific strengths identified.</li>";

  let weaknessesHtml = weaknesses.length > 0 
    ? weaknesses.map(w => `<li><strong>${w.qHeader}</strong> (Score: ${w.score})</li>`).join("")
    : "<li>No major weaknesses identified.</li>";

  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
      <div style="background-color: #34495e; color: white; padding: 20px; text-align: center; border-radius: 8px 8px 0 0;">
        <h2 style="margin: 0;">Submission Report</h2>
        <p style="margin: 5px 0 0 0;">${examName}</p>
      </div>
      
      <div style="padding: 20px; border: 1px solid #ddd; border-top: none; border-radius: 0 0 8px 8px;">
        <p><strong>Student:</strong> ${studentName} (${studentEmail})</p>
        <p><strong>Total Score:</strong> <span style="font-size: 20px; color: #2980b9;">${totalScore}</span></p>
        ...
        <h3 style="color: #c0392b;">⚠️ Areas for Improvement</h3>
        <ul style="padding-left: 20px;">
          ${weaknessesHtml}
        </ul>
        
        <div style="margin-top: 30px; font-size: 12px; color: #7f8c8d; border-top: 1px solid #eee; padding-top: 10px;">
          <p>Generated by AI Grading System - ${new Date().toLocaleString()}</p>
        </div>
      </div>
    </div>
  `;
